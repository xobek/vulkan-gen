...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    VkPresentModeKHR present_mode = VK_PRESENT_MODE_FIFO_KHR; // FIFO always exists
    ...
    swapchain_extent.width = VCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = VCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)vallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)vallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
    ...
    // destroy the views, not the images
    for (u32 i = 0; i < swapchain->image_count; ++i) {
