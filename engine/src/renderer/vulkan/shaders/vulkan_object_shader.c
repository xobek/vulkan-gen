...
b8 vulkan_object_shader_create(vulkan_context* context, vulkan_object_shader* out_shader) {
    ...
    vzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    vzero_memory(stage_create_infos, sizeof(stage_create_infos));
    for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; ++i) {
...
void vulkan_object_shader_update_object(vulkan_context* context, struct vulkan_object_shader* shader, geometry_render_data data) {
    ...
    vzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    f32 s = (vsin(accumulator) + 1.0f) / 2.0f;  // scale from -1, 1 to 0, 1
