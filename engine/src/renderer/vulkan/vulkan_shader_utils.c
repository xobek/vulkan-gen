...
b8 create_shader_module(
    ...
    vzero_memory(&shader_stages[stage_index].create_info, sizeof(VkShaderModuleCreateInfo));
    ...
    vzero_memory(&shader_stages[stage_index].shader_stage_create_info, sizeof(VkPipelineShaderStageCreateInfo));
    ...
    if (file_buffer) {
        vfree(file_buffer, sizeof(u8) * size, MEMORY_TAG_STRING);
        file_buffer = 0;
    }
